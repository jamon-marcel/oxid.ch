...
import Upload from "@/components/profile/images/Upload.vue";
  ...
  methods: {
    ...
    deleteAsset(image, event) {
      if (confirm("Please confirm!")) {
        let uri = `/api/profile/image/destroy/${image.id}`;
    ...
    toggleAsset(asset, event) {
      let uri = `/api/profile/image/status/${asset.id}`;
      let el = this.progress(event.target);
      this.axios.get(uri).then(response => {
    ...
    updateAsset(asset, event) {
      let uri = `/api/profile/image/update/${asset.id}`;
