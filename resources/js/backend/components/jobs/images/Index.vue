...
import Upload from "@/components/jobs/images/Upload.vue";
  ...
  methods: {
    ...
    deleteAsset(image, event) {
      if (confirm("Please confirm!")) {
        let uri = `/api/job/image/destroy/${image.id}`;
    ...
    toggleAsset(asset, event) {
      let uri = `/api/job/image/status/${asset.id}`;
      let el = this.progress(event.target);
      this.axios.get(uri).then(response => {
    ...
    updateAsset(asset, event) {
      let uri = `/api/job/image/update/${asset.id}`;
